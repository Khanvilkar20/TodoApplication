...
export default function EditTaskScreen({ route, navigation }: Props) {
  ...
  const initialScheduledDateObj = new Date(task.dateTime);
  const initialDeadlineDateObj = new Date(task.deadline);

  const [title, setTitle] = useState(task.title);
  const [description, setDescription] = useState(task.description ?? '');
  const [scheduledDate, setScheduledDate] = useState(
    !isNaN(initialScheduledDateObj.getTime())
      ? formatDate(initialScheduledDateObj)
      : formatDate(new Date())
  );
  const [scheduledTime, setScheduledTime] = useState(
    !isNaN(initialScheduledDateObj.getTime())
      ? formatTime(initialScheduledDateObj)
      : formatTime(new Date())
  );
  const [deadlineDate, setDeadlineDate] = useState(
    !isNaN(initialDeadlineDateObj.getTime())
      ? formatDate(initialDeadlineDateObj)
      : formatDate(new Date(Date.now() + 24 * 60 * 60 * 1000))
  );
  const [deadlineTime, setDeadlineTime] = useState(
    !isNaN(initialDeadlineDateObj.getTime())
      ? formatTime(initialDeadlineDateObj)
      : '18:00'
  );
  ...
  const validate = (): {
    ...
    if (parsedScheduled && parsedDeadline) {
      if (parsedDeadline.getTime() < parsedScheduled.getTime()) {
        newErrors.deadline = 'Deadline cannot be earlier than scheduled date & time';
      }
    }
